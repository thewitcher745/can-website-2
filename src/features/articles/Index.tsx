...
const IndexCard = ({
  ...
  return (
    ...
    >
      <div className="h-full relative p-2 w-30 sm:w-full sm:h-1/2 overflow-hidden rounded-sm sm:rounded-t-sm mb-2">
        <Image
          fill
      ...
      <div className="w-1/2 h-full sm:h-1/2 sm:w-full sm:px-3 sm:pb-3 grow-1 flex flex-col justify-between">
        ...
        <div className="flex justify-between">
          ...
          <div className="text-sm text-primary underline hover:text-primary-soft transition">
            More →
          </div>
        </div>
      </div>
...
const Index: React.FC<ListingsProps> = ({
  items,
  baseHref,
  filterTags,
  onRemoveTag,
    ...
    <>
      ...
      <div className="flex flex-col sm:grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
        {items.length === 0 ? (
          ...
          items.map((article) => (
            <IndexCard article={article} baseHref={baseHref} />
          ))
        )}
      </div>
    </>
